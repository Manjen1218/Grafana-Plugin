...
def parse_test_block(block_lines, subtest_results):
    ...
    for subtest in subtest_results:
        ...
        end_pattern = re.compile(rf'\s+test end\s+', re.IGNORECASE)
...
def parse_log(log_path):
    ...
    summary_capture_start_pattern = re.compile(r'\bCapture start\b', re.IGNORECASE)
    ...
    for _f_block in summary_tests:
        ...
        for beg, beg_line in enumerate(block):
            if summary_capture_start_pattern.search(beg_line):
                block_capture_start = beg
        results, total_result = parse_test_block(block[block_capture_start:], subtest_results)
        ...
        if results:
            ...
            for r in results:
                ...
                r["absolute_line_number"] = start_line + block_capture_start + r["line_number"]
                r["absolute_end_line_number"] = (
                    start_line + block_capture_start + r["end_line_number"] if r["end_line_number"] is not None else None
                )
                del r["line_number"]
